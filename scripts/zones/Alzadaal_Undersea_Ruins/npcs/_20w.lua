...
-- Door: Gilded Gateway (Bhaflau Remnants Entrance)
-- !pos 620 -2 -200 72
-----------------------------------
...
function onEventUpdate(player,csid,option,target)
    ...
    local instanceid = bit.rshift(option, 19) + 67
    ...
    player:createInstance(instanceid, 75)

end

function onEventFinish(player,csid,option,target)
    if (csid == 409 and option == 4) or csid == 116 then
        player:setPos(0,0,0,0,75)
    end
end
        ...
        if party ~= nil then
            for i,v in ipairs(party) do
                if v:getID() ~= player:getID() and v:getZoneID() == player:getZoneID() then
                    ...
                    v:setLocalVar("SalvageBhaflau", 1)
                end
            end
        end
    else
